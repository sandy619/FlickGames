...
public class FortuneWheelManager : MonoBehaviour
{
    ...
    {
        if (DateTime.Now >= TimeOfNextSpin)
        {
            Debug.Log("can spin");
            ...
            TimeOfNextSpin = DateTime.Now.AddSeconds(dailyTimer);
            PlayerPrefs.SetString("NextSpinDate",TimeOfNextSpin.ToString());
        }
        else
        {
            Debug.Log("cannot spin");
    ...
    void Update ()
    {
        ...
        if (DateTime.Now >= DateTime.Parse(PlayerPrefs.GetString("NextSpinDate",TimeOfNextSpin.ToString())))
            dailyCooldownComplete = false;
        else
            dailyCooldownComplete = true;

        // Make turn button non interactable if user has not enough money for the turn
        if (_isStarted || CurrentCoinsAmount < TurnCost|| dailyCooldownComplete) {
